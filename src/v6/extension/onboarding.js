...
function render() {
  ...
  if (state?.nativeConnected && state?.hostVersion === state?.version) {
    ...
    $("helperStatus").textContent = `Helper ${state.hostVersion || "unknown"} connected — V6.1.3 update recommended`;
...
$("helper").onclick = () => {
  chrome.tabs.create({url:state?.setupUrl || "https://github.com/viesca272/AnimeKai-RPC/releases/tag/v6.1.3"});
};
